...
public class WordLadder {
	public static void main(String[] args) throws java.lang.Exception {
		...
		System.out.println(ladderLength(w, f, list));
		f = "dog";
		System.out.println(ladderLength(w, f, list));
		f = "dog";
		w = "dot";
		...
		w = "cog";
		System.out.println(ladderLength(w, f, list));
		list = Arrays.asList("slit", "bunk", "wars", "ping", "viva", "wynn", "wows", "irks", "gang", "pool", "mock",
		...
		System.out.println(ladderLength(w, f, list));
		System.out.println("------------");
		System.out.println(ladderLength1(w, f, list));

		System.out.println(ladderLengthArray(w, f, list));
	...
	public static int ladderLength1(String beginWord, String endWord, List<String> wordList) {
		...
		wordList.forEach( // O(N)
				word -> {
					for (int i = 0; i < L; i++) { // O(Length)
						// Key is the generic word
						// Value is a list of words which have the same intermediate generic word.
						String newWord = word.substring(0, i) + '*' + word.substring(i + 1, L); // take order(Length)
	...
	public static int ladderLengthArray(String beginWord, String endWord, List<String> wordList) {
		int count = 1;
		...
		while (queue.isEmpty() == false) {
			int n = queue.size();// run to a level
		...
		while (queue.isEmpty() == false) {
			...
			for (int k = 0; k < n; k++) {
				...
				for (int i = 0; i < temp.length(); i++) {
					...
					String last = temp.substring(i + 1);
					for (int j = 0; j < 26; j++) {
						char ch = (char) ('a' + j);
						if (ch == c)
							continue;
						String con = pre + ch + last;
						if (wordList.contains(con) && !set.contains(con)) {
							queue.add(con);
							set.add(con);
						}
